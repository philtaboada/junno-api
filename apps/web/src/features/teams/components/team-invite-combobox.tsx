...
import { TeamAccessRoleMenu } from '@/features/teams/components/team-access-role-menu';
...
type TeamInviteComboboxProps = {
  teamId: string;
  inviteRole: TeamAccessRole;
  onInviteRoleChange: (role: TeamAccessRole) => void;
  query: string;
...
export function TeamInviteCombobox({
  teamId,
  inviteRole,
  onInviteRoleChange,
  query,
        ...
        <div className="absolute top-[calc(100%+4px)] z-50 max-h-64 w-full overflow-y-auto rounded-lg border border-border/80 bg-popover p-1 shadow-md ring-1 ring-foreground/10">
          ...
          {!isSearching && !hasResults && query.trim().length > 0 ? (
            <p className="px-3 py-2 text-sm text-muted-foreground">
              {query.includes('@')
                ? 'Pulsa Invitar para enviar invitación por email.'
